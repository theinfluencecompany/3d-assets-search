...
/**
 * Shape of each raw entry in the poly.pizza export files.
 * Derived from RawAssetSchema: `creator` is omitted (injected from the file name),
 * and `animationClips` gets a `.default([])` since the raw file may omit the field.
 */
...
function main() {
  ...
  console.log(
    `✅ ${assets.length} assets | ${index.meta.animatedAssets} animated | ${index.meta.totalTokens} tokens`,
  );
